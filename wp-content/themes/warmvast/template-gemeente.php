...
while ( have_posts() ) :
	...
	if ( ! $gemeente ) :
		...
		// No icon tile here: it was always the same generic 'map' mark on every
		// gemeente page, identifying nothing the eyebrow text doesn't already
		// say. Kept for maatregel pages, where it genuinely distinguishes which
		// of four otherwise-similar pages you're on.
		warmvast_the_hero(
			array(
				'variant'    => 'gemeente',
				'breadcrumb' => array(
					array( 'label' => 'Home', 'url' => home_url( '/' ) ),
					array( 'label' => 'Gemeentes', 'url' => home_url( '/gemeentes/' ) ),
					array( 'label' => $gemeente['naam'] ),
				),
				'eyebrow'    => 'ISDE-subsidie in regio Zaandam',
				'title'      => 'ISDE-subsidie voor isolatie in ' . $gemeente['naam'],
				'lead'       => 'Warmvast helpt woningeigenaren in ' . $gemeente['naam'] . ' met technische opname, heldere m²-berekening en een realistische ISDE-indicatie — ' . lcfirst( $gemeente['positie'] ) . '.',
				'actions'    => array(
					array(
				...
				'aside'      => array(
					'type'   => 'data-card',
					'kicker' => 'In het kort',
					// 'Ligging' (a full sentence, not a figure) is deliberately
					// left out of this data-surface card -- it's already the
					// hero's own lead sentence above, and the fuller "De regio"
					// section further down the page.
					'rows'   => array(
						array(
							'label' => 'Inwoners',
							'value' => $gemeente['inwoners'],
						),
						array(
							'label' => 'Kernen',
							'value' => (string) count( $gemeente['kernen'] ),
						),
					),
					'note'   => 'ISDE-tarieven zijn landelijk, ook in ' . $gemeente['naam'] . '.',
				),
			)
		);
		?>
		...
		<section class="section section--surface">
			<div class="container grid grid--2" style="align-items:start;gap:clamp(1.5rem,4vw,3rem)">
				...
				<div class="card" data-reveal>
					...
					<p class="u-note">Warmvast isoleert woningen in heel <?php echo esc_html( $gemeente['naam'] ); ?>, van de hoofdkern tot de kleinere buurtschappen.</p>
				</div>
			</div>
		</section>
